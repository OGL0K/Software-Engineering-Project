...
public class CWReq1 extends BaseQuery{
	...
	public ArrayList<Film> getActual() throws SQLException {
		...
		ArrayList<Film> TitlesstartwithdFilmIDs  = new ArrayList<Film>();
		...
		for(Film x: films) {
			if(x.getTitle().startsWith("D")) {
				TitlesstartwithdFilmIDs.add(x);
			}
		}
	...
	public void printOutput() throws SQLException{
		ArrayList<Film> TitlesstartwithdFilmIDs = getActual();
		System.out.println("The film titles whose titles start with 'D':\n");
		for(Film f:TitlesstartwithdFilmIDs) {
