...
public class CWReq2 extends BaseQuery{
	...
	public ArrayList<Long> getActual() throws SQLException {
			ArrayList<Film> films = new ArrayList<Film>();
			ArrayList<Film> documentaryFilmIDs  = new ArrayList<Film>();
			ArrayList<Long> documentaryFilms = new ArrayList<Long>();
			Film f;
			...
			for(Film x: films) {
				if(x.getDescription().contains("Documentary")) {
					documentaryFilmIDs.add(x);
				}
			}
			long total = documentaryFilmIDs.stream().count();
			
			documentaryFilms.add(total);
	...
	public int getNumber() throws SQLException {
		...
		return documentaryFilmIDs.size();
	}
	...
	public void printOutput() throws SQLException{
		System.out.println("Total: " + getNumber());
